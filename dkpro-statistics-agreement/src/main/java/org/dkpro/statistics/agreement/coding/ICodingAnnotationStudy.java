...
/**
 * Data model for coding studies. Coding studies are characterized by a set of fixed annotation
 * items which a number of (human) raters are asked code with a certain category. The individual
 * annotations represent the annotation units. The set of annotation units of an annotation item is
 * represented by the {@link IAnnotationItem} interface. Note that this interface allows reusing
 * existing data representations by implementing the required methods. Use
 * {@link CodingAnnotationStudy} as a default implementation if no existing data representation
