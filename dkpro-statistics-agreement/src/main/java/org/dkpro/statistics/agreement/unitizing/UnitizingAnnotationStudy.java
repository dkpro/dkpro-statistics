...
/**
 * Default implementation of the {@link IUnitizingAnnotationStudy} interface. Instantiate this class
 * for representing the annotation units of a unitizing study (i.e., an annotation setup in which
 * the (human) raters are asked to identify the boundaries of the annotation units themselves). The
 * standard way of representing the annotation units is using the
 * {@link #addUnit(long, long, int, Object)} method for each identified segment by each rater.
 * 
    ...
    implements IUnitizingAnnotationStudy, Serializable
{
     ...
     * The basic setup of a unitizing study is identifying units within a given continuum. The
     * continuum is initialized to start at position 0 and end at the given length.
     */
     ...
     * Initializes and empty annotation study for a unitizing task with the given number of raters.
     * The basic setup of a unitizing study is identifying units within a given continuum. The
     * continuum is initialized to start at the given position and end after the specified length
    ...
    /**
     * Add the given annotation unit to this study. The specified unit should never be null. When
     * relying on the default implementation {@link UnitizingAnnotationUnit}, it is recommended to
     * use {@link #addUnit(long, long, int, Object)} instead.
     */
    ...
    /**
     * Creates a new {@link UnitizingAnnotationUnit} and adds it to the present annotation study.
     * The parameters define the position of the unit within the continuum (indicated by the
     * offset/start position and the length of the unit), the rater who identified this unit and the
     * category assigned to it. If the unitzing study does not distinguish any categories, use some
     * dummy object, such as "X" to distinguish identified units from gaps (i.e., units whose
     * category is null).
     */
    ...
    /**
     * Utility method for moving on the cursor of the given iterator until a unit of the specified
     * rater is returned.
     */
    ...
    /**
     * Utility method for moving on the cursor of the given iterator until a unit of the specified
     * rater and category is returned. Both the rater index and the category may be null if those
     * filter conditions are to be ignored.
     */
    public static IUnitizingAnnotationUnit findNextUnit(
            final Iterator<IUnitizingAnnotationUnit> units, int raterIdx, final Object category)
    {
        while (units.hasNext()) {
    ...
    /*
     * @Deprecated public int getSectionCount() { return sections.size(); }
     * 
     * @Deprecated public Iterable<Integer> getSectionBoundaries() { return sections; }
     */
    ...
    /*
     * public ICodingAnnotationStudy digitize() { CodingAnnotationStudy result = new
     * CodingAnnotationStudy(raters.size()); for (int i = begin; i < begin + length; i++)
     * result.addItem(annotations) return result; }
     */
