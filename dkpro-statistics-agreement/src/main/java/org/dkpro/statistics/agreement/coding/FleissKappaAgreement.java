    ...
    implements IChanceCorrectedAgreement, ICategorySpecificAgreement
{
    ...
    /**
     * Calculates the expected inter-rater agreement that assumes the same distribution for all
     * raters and annotations.
     * 
     * @throws NullPointerException
     *             if the annotation study is null.
     * @throws ArithmeticException
     *             if there are no items in the annotation study.
     */
    @Override
    public double calculateExpectedAgreement()
    {
        ...
        for (ICodingAnnotationItem item : study.getItems()) {
            ...
            for (Entry<Object, Integer> counts : annotationsPerCategory.entrySet()) {
                BigDecimal p = new BigDecimal(counts.getValue())
                        .divide(new BigDecimal(item.getRaterCount()), MathContext.DECIMAL128);
    ...
    /*
     * Modified version of Fleiss' kappa for measuring a chance-corrected inter-rater agreement for
     * a certain category. Note that the generalized kappa for multiple raters is a generalized pi
     * (see (Artstein/Poesio, 2008) for details). A prerequisite of this measure is an annotation
     * study that involves at least three categories. For studies with only two categories, the
     * measure is equivalent to the general kappa. <br><br> References:<ul> <li>Fleiss, J. L.:
     * Measuring nominal scale agreement among many raters. Psychological Bulletin 76(5):378-381,
     * Washington, DC: American Psychological Association, 1971. <li>Artstein, R. & Poesio, M.:
